...
object EffectsHomework2 {
  ...
  final case class Map[B, +A](f: B => A) extends IO[A] {
    def apply(value: B): IO[A] = Pure(f(value))
  }

  //final case class FlatMap[A, B](f: A => IO[B]) extends IO[A]
